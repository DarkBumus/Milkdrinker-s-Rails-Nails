...
/**
 * Handles the configuration management for Milkdrinker's Rails&Nails, organizing configuration files into separate categories.
...
public class ModConfigurationBase extends Configuration {
    ...
    public ModConfigurationBase(File mainFile) {
        ...
        File subDir = new File(configDir, "mdrailsnails");

        // Create the config directory if it doesn't exist
        if (!subDir.exists()) {
            if (subDir.mkdirs()) {
                MDRailsNails.logger
                    .warn("[Milkdrinker's Rails&Nails] Failed to create config directory:{}", subDir.getAbsolutePath());
            }
        }
