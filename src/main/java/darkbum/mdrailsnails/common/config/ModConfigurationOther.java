...
public class ModConfigurationOther {
    ...
    public static void init(Configuration config) {
        ...
        for (int i = 0; i < loadedCloudLevel.length; i++) {
            ...
            if (pair.length != 2) {
                MDRailsNails.logger.warn("Invalid key-value pair at DimCloudLevel[{}]", i);
            } else {
                int dim, level;
                try {
                    dim = Integer.parseInt(pair[0]);
                } catch (NumberFormatException e) {
                    MDRailsNails.logger.warn("Cannot parse DimensionID \"{}\" to integer point at DimCloudLevel line {}", pair[0], i + 1);
                    break;
                }
                try {
                    level = Integer.parseInt(pair[1]);
                } catch (NumberFormatException e) {
                    MDRailsNails.logger.warn("Cannot parse CloudLevel \"{}\" to integer point at DimCloudLevel line {}", pair[1], i + 1);
                    break;
                }
