...
public class BlockRailOneWayDetector extends BlockRailDetector implements IModeableRail {

    private Block turnedBlock;
    private String modeMessageKey;

    public BlockRailOneWayDetector(String name, CreativeTabs tab) {
        super();
        setBlockName(name);
        setCreativeTab(tab);
    ...
    public void onMinecartPass(World world, EntityMinecart cart, int x, int y, int z) {
        handleOneWayCartDetectingBehavior(world, cart, x, y, z);
    }
    ...
    public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z, EntityPlayer player) {
        ...
        if ((block == one_way_detector_rail_r)
            || (block == one_way_detector_rail_l)) {
            return new ItemStack(one_way_detector_rail_r);
        }
        return null;
    }
    ...
    public Item getItemDropped(int meta, Random random, int fortune) {
        return getItemFromBlock(one_way_detector_rail_r);
    }
