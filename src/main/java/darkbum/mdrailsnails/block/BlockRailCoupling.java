...
public class BlockRailCoupling extends BlockRailPowered implements IModeableRail {

    private Block turnedBlock;
    private String modeMessageKey;

    public BlockRailCoupling(String name, CreativeTabs tab) {
        super();
        setBlockName(name);
        setCreativeTab(tab);
    ...
    public void onMinecartPass(World world, EntityMinecart cart, int x, int y, int z) {
        handleCouplingCartBehavior(world, cart, x, y, z);
    }
    ...
    public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z, EntityPlayer player) {
        ...
        if ((block == coupling_rail)
            || (block == decoupling_rail)) {
            return new ItemStack(coupling_rail);
        }
        return null;
    }
    ...
    public Item getItemDropped(int meta, Random random, int fortune) {
        return getItemFromBlock(coupling_rail);
    }
